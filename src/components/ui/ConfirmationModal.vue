    ...
    <template #container="{ message, acceptCallback, rejectCallback }">
      <div class="flex flex-col items-center p-5 bg-surface-0 dark:bg-surface-700 rounded-md">
        <div
          ...
          <i class="fa-solid fa-exclamation-circle text-5xl"></i>
        </div>
        <span class="font-bold text-2xl block mb-2 mt-4">{{ message.header }}</span>
        <p class="mb-0">{{ message.message }}</p>
        <div class="flex items-center gap-2 mt-4">
          <Button :label="message.rejectLabel" outlined @click="rejectCallback" class="w-[8rem]"></Button>
          <Button :label="message.confirmLabel" @click="acceptCallback" class="w-[8rem]"></Button>
        </div>
      </div>
    </template>
  </ConfirmDialog>
